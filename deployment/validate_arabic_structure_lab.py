...
except Exception as e: fail(f'could not parse evaluated A1 course result: {e}; stdout={r.stdout!r}')
if (a1.get('count'),a1.get('first'),a1.get('last'))!=(750,'a1e001','a1e750'): fail(f'A1 runtime course invalid: {a1}')
...
index_lines=[x.strip() for x in pages.splitlines() if 'sed -i' in x and '_site/arabic/index.html' in x]
if len(index_lines)!=1: fail(f'expected exactly one Arabic index injection line, found {len(index_lines)}')
order=re.findall(r'<script src="\./([^"]+)"></script>',index_lines[0])
...
if len(order)!=len(set(order)): fail('duplicate production script detected')
if any(x not in order for x in required_order): fail('missing required production script')
if [order.index(x) for x in required_order]!=sorted(order.index(x) for x in required_order): fail('production script dependency order is invalid')
ok('production script order is deterministic and dependency-safe')

audio=read('arabic/audio-service.js')
if not all(x in audio for x in ('ARABIC_AUDIO_SERVICE','SpeechSynthesisUtterance','chooseVoice')): fail('AudioService contract missing')
word_audio=read('arabic/word-audio.js')
if 'ARABIC_AUDIO_SERVICE' not in word_audio or 'data-speak-ar' not in word_audio or 'SpeechSynthesisUtterance' in word_audio: fail('word audio bypasses AudioService')
...
for x in ('courseSentenceWord','sentenceHtml','data-course-word','Arabic · tap any word','A1 Course','ARABIC_COURSE_OPEN',"openCourse('A1')"):
    if x not in course: fail(f'Course interaction/A1 entry missing: {x}')
...
if 'ARABIC_AUDIO_SERVICE' not in progressive or 'SpeechSynthesisUtterance' in progressive: fail('progressive word replay bypasses AudioService')
...
if 'arabicFallbackHtml' not in pager or 'data-speak-ar' not in pager: fail('AI Corpus fallback tokenization missing')
...
for x in ('Pronunciation','Meaning','Dictionary Form · Stem · Root','Grammar','Conjugation Table','Case Table','Current Form','nominalCurrentCell','ARABIC_WORD_PANEL_OPEN'):
    if x not in panel: fail(f'full word-panel contract missing: {x}')
if 'count=Math.min' in panel: fail('word panel still requires repeated taps')
...
for x in ('مَكْتَبَة','Sound Masculine Plural','Sound Feminine Plural','Broken Plural','Nominative','Accusative','Genitive'):
    if x not in decl: fail(f'nominal declension contract missing: {x}')
