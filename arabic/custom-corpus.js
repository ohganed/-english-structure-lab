...
function saveAI(d){let a=[];try{a=JSON.parse(localStorage.getItem(AIK)||'[]')}catch{}const key=d.originalText||d.title||String(Date.now());a=[{key,title:d.title||'AI analyzed text',createdAt:new Date().toISOString(),d},...a.filter(x=>x.key!==key)].slice(0,200);localStorage.setItem(AIK,JSON.stringify(a));try{window.ARABIC_KB?.ingestDocument?.(d)}catch(e){console.warn('Arabic KB ingest skipped',e)}}
...
const load=document.querySelector('#load');if(load){load.textContent='Save to AI Corpus & open';load.addEventListener('click',ev=>{ev.preventDefault();ev.stopImmediatePropagation();try{let d=normalizeScenes(parseJson());validate(d);L=d;si=0;saveAI(d);render();close('import');toast('Saved to AI Corpus + Knowledge Base')}catch(e){const st=document.querySelector('#status');if(st)st.textContent=e.message}},true)}
})();